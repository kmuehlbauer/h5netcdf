...
class BaseVariable(object):
    ...
    def _lookup_dimensions(self):
        ...
        if (
            ...
            order_dim = {
                value.dimid: key for key, value in self._parent._all_dimensions.items()
            }
            return tuple(
    ...
    def _attach_coords(self):
        ...
        coord_ids = np.array(
            [self._parent._all_dimensions[d].dimid for d in dims], "int32"
        )
        if len(coord_ids) > 1:
...
class Group(Mapping):
    ...
    def __init__(self, parent, name):
        ...
        if self._root._phony_dims_mode is not None:
            ...
            labeled_dims = Counter(
                [d.maxsize for d in self._dimensions.values() if not d.isphony]
            )
    ...
    def _create_dim_scale(self, dim, size):
        ...
        h5ds.attrs["_Netcdf4Dimid"] = np.array(
            self._dimensions[dim].dimid, dtype=np.int32
        )
