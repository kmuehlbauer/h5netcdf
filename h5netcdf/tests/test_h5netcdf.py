...
def write_h5netcdf(tmp_netcdf, compression="gzip", format="NETCDF4", backend=None):
    if backend is None:
        backend = "h5py"

    ds = h5netcdf.File(tmp_netcdf, mode="w", format=format, backend=backend)
    intf = np.int64 if ds.data_model == "NETCDF4" else np.int32
...
def read_legacy_netcdf(tmp_netcdf, read_module, write_module, backend="h5py"):
    ...
    if (
        backend is not None
        and "h5py" in backend
        and is_h5py_char_working(tmp_netcdf, "z")
    ):
        ds = read_module.Dataset(tmp_netcdf, "r", backend=backend)
        v = ds.variables["z"]
        assert array_equal(v, _char_array)
...
def test_roundtrip_local(tmp_local_netcdf, wmod, rmod, bmod, decode_vlen, monkeypatch):
    ...
    if "h5netcdf" in wmod:
        _ = pytest.importorskip("h5py", reason="requires h5py")
    wmod = pytest.importorskip(wmod, reason=f"requires {wmod}")
    if "h5netcdf" in rmod and "h5py" in bmod:
        _ = pytest.importorskip("h5py", reason="requires h5py")
...
def test_roundtrip_h5netcdf(
    tmp_netcdf, write_backend, read_backend, decode_vlen_strings
):
    write_h5netcdf(tmp_netcdf, backend=write_backend.name)
    read_h5netcdf(tmp_netcdf, h5netcdf, decode_vlen_strings, backend=read_backend.name)
...
def test_dump_netcdf4_vs_h5netcdf(
    ...
    new_tmp_local_netcdf = tmp_local_netcdf[:-3] + "_nc4.nc"
    import shutil

    shutil.copy(tmp_local_netcdf, new_tmp_local_netcdf)
...
def test_repr(tmp_netcdf, write_backend):
    write_h5netcdf(tmp_netcdf, backend=write_backend.name)
    f = h5netcdf.File(tmp_netcdf, "a", backend=write_backend.name)
...
def test_attrs_api(tmp_netcdf, write_backend, read_backend):
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.attrs["conventions"] = "CF"
        ds.attrs["empty_string"] = ds._h5py.Empty(dtype=np.dtype("|S1"))
        ds.dimensions["x"] = 1
        v = ds.create_variable("x", ("x",), "i4")
        v.attrs.update({"units": "meters", "foo": "bar"})
    assert ds._closed
    with h5netcdf.File(tmp_netcdf, "r", backend=read_backend.name) as ds:
...
def test_shape_is_tied_to_coordinate(tmp_netcdf, write_backend, read_backend):
    with h5netcdf.legacyapi.Dataset(tmp_netcdf, "w", backend=write_backend.name) as ds:
    ...
    with h5netcdf.legacyapi.Dataset(tmp_netcdf, "r", backend=read_backend.name) as ds:
        assert ds["yvar"].shape == (10,)
        assert ds["xvar"].shape == (10,)


def test_optional_netcdf4_attrs(tmp_netcdf, write_backend, read_backend):
    # create with h5py/h5pyd
    with write_backend.mod.File(tmp_netcdf, "w") as f:
...
@requires_h5py_h5pyd
def test_error_handling(tmp_netcdf, write_backend):
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.dimensions["x"] = 1
...
@requires_h5py_h5pyd
def test_decode_string_error(tmp_netcdf, write_backend):
    write_h5netcdf(tmp_netcdf, backend=write_backend.name)
    with raises(TypeError, match="keyword argument is not allowed"):
        with h5netcdf.legacyapi.Dataset(
            tmp_netcdf, "r", decode_vlen_strings=True, backend=write_backend.name
        ) as ds:
            assert ds.name == "/"
...
def test_invalid_netcdf4(tmp_netcdf, write_backend, read_backend):
    with write_backend.mod.File(tmp_netcdf, "w") as f:
    ...
    with h5netcdf.File(
        tmp_netcdf, "r", phony_dims="sort", backend=read_backend.name
    ) as dsr:
    ...
    with h5netcdf.File(
        tmp_netcdf, "r", phony_dims="access", backend=read_backend.name
    ) as dsr:
        for i, grp in enumerate(grps):
            var = dsr[grp].variables
            check_invalid_netcdf4(var, i)

    if not tmp_netcdf.startswith(remote_h5) and has_netCDF4:
        import netCDF4

        # netcdf4 package does not work with remote HDF5 files
        with netCDF4.Dataset(tmp_netcdf, "r") as dsr:
            for i, grp in enumerate(grps):
                var = dsr[grp].variables
                check_invalid_netcdf4(var, i)
    ...
    with raises(ValueError, match="unknown value"):
        with h5netcdf.File(
            tmp_netcdf, "r", phony_dims="srt", backend=read_backend.name
        ) as ds:
            pass


@requires_h5py_h5pyd
def test_fake_phony_dims(tmp_netcdf, write_backend):
    # tests writing of dimension with phony naming scheme
    # see https://github.com/h5netcdf/h5netcdf/issues/178
    with h5netcdf.File(tmp_netcdf, mode="w", backend=write_backend.name) as ds:
...
def test_invalid_netcdf4_mixed(tmp_netcdf, write_backend, read_backend):
    with write_backend.mod.File(tmp_netcdf, "w") as f:
        var, var2 = create_invalid_netcdf_data()
        for k, v in var.items():
...
def test_hierarchical_access_auto_create(tmp_netcdf, write_backend, read_backend):
    ds = h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name)
    ...
    ds = h5netcdf.File(tmp_netcdf, "r", backend=read_backend.name)
    assert set(ds) == {"foo"}
    assert set(ds["foo"]) == {"bar", "baz", "hello"}
    ds.close()


def test_Netcdf4Dimid(tmp_netcdf, write_backend, read_backend):
    # regression test for https://github.com/h5netcdf/h5netcdf/issues/53
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as f:
        f.dimensions["x"] = 1
        g = f.create_group("foo")
        g.dimensions["x"] = 2
        g.dimensions["y"] = 3

    with read_backend.mod.File(tmp_netcdf, "r") as f:
...
@requires_netCDF4
@pytest.mark.local
def test_reading_str_array_from_netCDF4(tmp_netcdf, read_backend, decode_vlen_strings):
    ...
    ds = h5netcdf.File(
        tmp_netcdf, "r", backend=read_backend.name, **decode_vlen_strings
    )

    v = ds.variables["bar"]
    # pyfive always encodes strings
    if read_backend.name == "pyfive" or getattr(ds, "decode_vlen_strings", True):
...
def test_nc_properties_new(tmp_netcdf, write_backend, read_backend):
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as _:
        pass
    with read_backend.mod.File(tmp_netcdf, "r") as f:
...
@pytest.mark.local
def test_failed_read_open_and_clean_delete(tmpdir, read_backend):
    ...
    try:
        with h5netcdf.File(path, "r", backend=read_backend.name) as ds:
            assert ds
    except OSError:
        pass
...
def test_create_variable_matching_saved_dimension(
    tmp_netcdf, write_backend, read_backend
):
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as f:
        f.dimensions["x"] = 2
    ...
    with h5netcdf.File(tmp_netcdf, "a", backend=write_backend.name) as f:
...
@requires_h5py_h5pyd
def test_invalid_netcdf_error(tmp_netcdf, write_backend):
    if tmp_netcdf.startswith(remote_h5):
        pytest.skip("Remote HDF5 does not yet support LZF compression")
    with h5netcdf.File(
        tmp_netcdf, "w", invalid_netcdf=False, backend=write_backend.name
    ) as f:
        # valid
        f.create_variable(
...
def test_invalid_netcdf_okay(tmp_netcdf, write_backend, read_backend):
    if tmp_netcdf.startswith(remote_h5):
        pytest.skip("h5pyd does not support NumPy complex dtype yet")
    with warns(UserWarning, match="invalid netcdf features"):
        with h5netcdf.File(
            tmp_netcdf, "w", invalid_netcdf=True, backend=write_backend.name
        ) as f:
            f.create_variable(
                "lzf_compressed", data=[1], dimensions=("x"), compression="lzf"
            )
    ...
    with h5netcdf.File(tmp_netcdf, "r", backend=read_backend.name, **kwargs) as f:
...
@requires_netCDF4
def test_invalid_netcdf_overwrite_valid(tmp_local_netcdf, local_read_backend):
    import netCDF4

    if local_read_backend.name == "pyfive":
    ...
    with h5netcdf.File(
        tmp_local_netcdf, "r", backend=local_read_backend.name, **kwargs
    ) as f:
        np.testing.assert_equal(f["lzf_compressed"][:], [1])
        assert f["complex"][...] == 1j
        assert f.attrs["complex_attr"] == 1j
        ...
        with expected_errors:
            np.testing.assert_equal(f["scaleoffset"][:], [1])
    with local_read_backend.mod.File(tmp_local_netcdf, "r") as f:
        assert "_NCProperties" not in f.attrs
...
def test_invalid_then_valid_no_ncproperties(tmp_netcdf, write_backend, read_backend):
    with warns(UserWarning, match="invalid netcdf features"):
        with h5netcdf.File(
            tmp_netcdf, "w", backend=write_backend.name, invalid_netcdf=True
        ):
            pass
    with h5netcdf.File(tmp_netcdf, "a", backend=write_backend.name):
        pass
    with read_backend.mod.File(tmp_netcdf, "r") as f:
...
def test_creating_variables_with_unlimited_dimensions(
    ...
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as f:
        ...
        if not tmp_netcdf.startswith(remote_h5):
    ...
    # dimensions.
    with h5netcdf.File(tmp_netcdf, "r", backend=read_backend.name) as f:
...
@requires_write_backend
def test_writing_to_an_unlimited_dimension(tmp_netcdf, write_backend, read_backend):
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as f:
...
def test_reading_unlimited_dimensions_created_with_c_api(
    tmp_local_netcdf, local_read_backend
):
    import netCDF4
    ...
    with h5netcdf.File(tmp_local_netcdf, "r", backend=local_read_backend.name) as f:
...
def test_reading_unused_unlimited_dimension(tmp_netcdf, write_backend, read_backend):
    ...
    with h5netcdf.File(tmp_netcdf, "r", read_backend.name) as f:
        assert f.dimensions["x"].isunlimited()
        assert f.dimensions["x"].size == 5
...
@requires_h5py
@requires_netCDF4
def test_reading_special_datatype_created_with_c_api(
    tmp_local_netcdf, local_read_backend
):
...
@requires_h5py_ge_3_7_0
def test_nc4_non_coord(tmp_netcdf, write_backend, read_backend):
...
@requires_netCDF4
@requires_h5py
def test_overwrite_existing_file(tmp_local_netcdf, local_read_backend):
    import netCDF4
    ...
    # check attribute
    with h5netcdf.File(tmp_local_netcdf, "r", backend=local_read_backend.name) as ds:
...
@requires_write_backend
def test_overwrite_existing_remote_file(tmp_netcdf, write_backend, read_backend):
    # create file with legacyapi
    with legacyapi.Dataset(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.createDimension("x", 10)
...
@requires_netCDF4
def test_create_attach_scales(
    tmp_local_netcdf, netcdf_write_module, local_read_backend
):
...
def test_detach_scale(tmp_netcdf, write_backend, read_backend):
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.dimensions["x"] = 2
        ds.dimensions["y"] = 2

    with h5netcdf.File(tmp_netcdf, "a", backend=write_backend.name) as ds:
...
@pytest.mark.xfail(reason="pyfive issue")
def test_is_scale(tmp_netcdf, write_backend, read_backend):
    with legacyapi.Dataset(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.createDimension("x", 10)
    with legacyapi.Dataset(tmp_netcdf, "r", backend=read_backend.name) as ds:
        assert ds.dimensions["x"]._isscale


@requires_h5py_h5pyd
def test_get_dim_scale_refs(tmp_netcdf, write_backend, read_backend):
    with legacyapi.Dataset(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.createDimension("x", 10)
    ...
    with legacyapi.Dataset(tmp_netcdf, "r", backend=read_backend.name) as ds:
        ...
        # restrict, since id's will differ for different backends
        if write_backend.name == read_backend.name:
            assert ds._h5file[refs[0][0]] == ds["test0"]._h5ds
            assert ds._h5file[refs[1][0]] == ds["test1"]._h5ds
        assert str(ds._h5file[refs[0][0]]) == str(ds["test0"]._h5ds)
        assert str(ds._h5file[refs[1][0]]) == str(ds["test1"]._h5ds)
...
@requires_write_backend
def test_no_circular_references(tmp_netcdf, write_backend, read_backend):
...
@requires_write_backend
def test_no_circular_references_py314(tmp_netcdf, write_backend, read_backend):
    # https://github.com/h5py/h5py/issues/2019
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.dimensions["x"] = 2
        ds.dimensions["y"] = 2
...
def test_more_than_7_attr_creation_track_order(tmp_netcdf, write_backend, track_order):
    with h5netcdf.File(
        tmp_netcdf, "w", track_order=track_order, backend=write_backend.name
    ) as h5file:
        for i in range(100):
            h5file.attrs[f"key{i}"] = i
            h5file.attrs[f"key{i}"] = 0
...
@requires_netCDF4
def test_group_names(tmp_local_netcdf, local_read_backend):
    import netCDF4
    ...
    with legacyapi.Dataset(
        tmp_local_netcdf, "r", backend=local_read_backend.name
    ) as ds:
        assert ds.name == "/"
        name = ""
        for i in range(10):
...
def test_legacyapi_endianess(tmp_netcdf, write_backend, read_backend):
    ...
    with read_backend.mod.File(tmp_netcdf, "r") as ds:
...
@pytest.mark.local
def test_bool_slicing_length_one_dim(tmp_netcdf, write_backend, read_backend):
    ...
    with legacyapi.Dataset(tmp_netcdf, "a", backend=write_backend.name) as ds:
...
@requires_h5py_h5pyd
def test_fancy_indexing(tmp_netcdf, write_backend):
...
def test_create_invalid_netcdf_catch_error(tmp_netcdf, write_backend):
...
@requires_netCDF4
@pytest.mark.local
def test_dimensions_in_parent_groups(tmpdir, write_backend, read_backend):
    import netCDF4
    ...
    with legacyapi.Dataset(leg_file, mode="w", backend=write_backend.name) as ds:
        ds0 = ds
        for i in range(10):
            ds = ds.createGroup(f"group{i:02d}")
        ds0.createDimension("x", 10)
    ...
    with h5netcdf.File(nc4_file, mode="r", backend=read_backend.name) as ds0:
        with h5netcdf.File(leg_file, mode="r", backend=read_backend.name) as ds1:
...
def test_array_attributes(tmp_netcdf, write_backend, read_backend):
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as ds:
        dt = write_backend.mod.string_dtype("utf-8")
        unicode = "unicodé"
        ds.attrs["unicode"] = unicode
        ds.attrs["unicode_0dim"] = np.array(unicode, dtype=dt)
        ds.attrs["unicode_1dim"] = np.array([unicode], dtype=dt)
        ds.attrs["unicode_array"] = np.array([unicode, "foobár"], dtype=dt)
        ds.attrs["unicode_list"] = [unicode]

        dt = write_backend.mod.string_dtype("ascii")
        ...
        dt = write_backend.mod.string_dtype("utf-8", 10)
        ...
        ds.attrs["unicode_fixed_array"] = np.array(
            [unicode.encode("utf-8"), "foobár".encode()], dtype=dt
        )

        dt = write_backend.mod.string_dtype("ascii", 10)
        ascii = "ascii"
    ...
    def check_all_attrs(ds, is_legacy=False, skip_unicode_fixed=False):
        unicode = "unicodé"
        ascii = "ascii"
        foobar = "foobar"

        # new or legacy api
        get_attr = lambda name: getattr(ds, name) if is_legacy else ds.attrs[name]

        # --- unicode ---
        assert get_attr("unicode") == unicode
        assert get_attr("unicode_0dim") == unicode
        assert get_attr("unicode_1dim") == unicode
        assert get_attr("unicode_array") == [unicode, "foobár"]
        assert get_attr("unicode_list") == unicode

        # --- ascii ---
        assert get_attr("ascii") == ascii
        assert get_attr("ascii_0dim") == ascii
        assert get_attr("ascii_1dim") == ascii
        assert get_attr("ascii_array") == [ascii, foobar]
        assert get_attr("ascii_list") == ascii

        # --- bytes ---
        assert get_attr("bytes") == ascii
        assert get_attr("bytes_0dim") == ascii
        assert get_attr("bytes_1dim") == ascii
        assert get_attr("bytes_array") == [ascii, foobar]
        assert get_attr("bytes_list") == ascii

        # --- fixed strings ---
        if not skip_unicode_fixed:
            assert get_attr("unicode_fixed") == unicode
            assert get_attr("unicode_fixed_0dim") == unicode
            assert get_attr("unicode_fixed_1dim") == unicode
            assert get_attr("unicode_fixed_array") == [unicode, "foobár"]

        assert get_attr("ascii_fixed") == ascii
        assert get_attr("ascii_fixed_0dim") == ascii
        assert get_attr("ascii_fixed_1dim") == ascii
        assert get_attr("ascii_fixed_array") == [ascii, foobar]

        assert get_attr("bytes_fixed") == ascii
        assert get_attr("bytes_fixed_0dim") == ascii
        assert get_attr("bytes_fixed_1dim") == ascii
        assert get_attr("bytes_fixed_array") == [ascii, foobar]

        # --- numbers & empty ---
        assert get_attr("int") == 1
        assert get_attr("intlist") == 1
        np.testing.assert_equal(get_attr("int_array"), np.arange(10))
        np.testing.assert_equal(get_attr("empty_list"), np.array([]))
        np.testing.assert_equal(get_attr("empty_array"), np.array([]))

    with h5netcdf.File(tmp_netcdf, mode="r", backend=read_backend.name) as ds:
        assert ds._h5py == read_backend.mod
        check_all_attrs(ds, skip_unicode_fixed=(read_backend.name == "h5pyd"))

    with legacyapi.Dataset(tmp_netcdf, mode="r", backend=read_backend.name) as ds:
        assert ds._h5py == read_backend.mod
        check_all_attrs(
            ds, is_legacy=True, skip_unicode_fixed=(read_backend.name == "h5pyd")
        )

    if write_backend.name == "h5py" and has_netCDF4:
        import netCDF4

        with netCDF4.Dataset(tmp_netcdf, mode="r") as ds:
            check_all_attrs(ds, is_legacy=True)


@requires_h5py_ge_3_7_0
def test_vlen_string_dataset_fillvalue(
    tmp_local_netcdf, decode_vlen_strings, local_read_backend
):
    ...
    with h5netcdf.File(
        tmp_local_netcdf, "r", **decode_vlen_strings, backend=local_read_backend.name
    ) as ds:
        assert ds._h5py.__name__ == local_read_backend.name
        decode_vlen = (
            decode_vlen_strings["decode_vlen_strings"]
            or local_read_backend.name == "pyfive"
        )
...
@requires_write_backend
def test_user_type_errors_new_api(tmp_netcdf, write_backend):
    enum_dict1 = dict(one=1, two=2, three=3, missing=254)
    enum_dict2 = dict(one=0, two=2, three=3, missing=255)
    # todo: acquire test.nc from tmpdir
    with legacyapi.Dataset("test.nc", "w", backend="h5py") as ds0:
...
@requires_write_backend
def test_user_type_errors_legacyapi(tmp_netcdf, write_backend):
    enum_dict1 = dict(one=1, two=2, three=3, missing=254)
    enum_dict2 = dict(one=0, two=2, three=3, missing=255)
    # todo: acquire test.nc from tmpdir
    with legacyapi.Dataset("test.nc", "w", backend="h5py") as ds0:
...
@requires_h5py_h5pyd
def test_enum_type_errors_new_api(tmp_netcdf, write_backend):
    enum_dict1 = dict(one=1, two=2, three=3, missing=254)
    enum_dict2 = dict(one=0, two=2, three=3, missing=255)
    with h5netcdf.File(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.dimensions = {"enum_dim": 4}
...
@requires_h5py_h5pyd
def test_enum_type_errors_legacyapi(tmp_netcdf, write_backend):
    enum_dict1 = dict(one=1, two=2, three=3, missing=254)
    enum_dict2 = dict(one=0, two=2, three=3, missing=255)
    with legacyapi.Dataset(tmp_netcdf, "w", backend=write_backend.name) as ds:
        ds.createDimension("enum_dim", 4)
...
def test_enum_type(tmp_netcdf, write_backend, read_backend):
    # test EnumType
    enum_dict = dict(one=1, two=2, three=3, missing=255)
...
def test_vltype_creation(
    tmp_netcdf, write_backend, read_backend, netcdf_write_module, dtype
):
    # skip for netCDF4 writer for remote hsds files
    if netcdf_write_module.__name__ == "netCDF4" and tmp_netcdf.startswith(remote_h5):
        pytest.skip("netCDF4 not working with h5pyd")

    with netcdf_write_module.Dataset(tmp_netcdf, "w") as ds:
    ...
    with h5netcdf.File(tmp_netcdf, "r", backend=read_backend.name) as ds:
        ...
        if read_backend.name != "pyfive":
            assert read_backend.mod.check_vlen_dtype(vlen_type.dtype) == np.dtype(dtype)
...
@requires_netCDF4
@requires_h5py_h5pyd
def test_compoundtype_creation(
    tmp_netcdf, write_backend, read_backend, netcdf_write_module
):
    ...
    kwargs = {}
    if netcdf_write_module.__name__ != "netCDF4":
        kwargs.update(backend=write_backend.name)
    with netcdf_write_module.Dataset(tmp_netcdf, "w", **kwargs) as ds:
    ...
    if read_backend.name == "pyfive":
        ...
        kwargs["unsupported_hdf5_features"] = "warn"
    with legacyapi.Dataset(tmp_netcdf, "r", backend=read_backend.name, **kwargs) as ds:
        cmptype = ds.cmptypes["cmp_t"]
...
@requires_netCDF4_ge_1_7_0
def test_nc_complex_compatibility(
    tmp_netcdf, write_backend, read_backend, netcdf_write_module
):
    if tmp_netcdf.startswith(remote_h5):
    ...
    if not tmp_netcdf.startswith(remote_h5):
        ...
        with netCDF4.Dataset(tmp_netcdf, "r", auto_complex=True) as ds:
            dtype = ds.cmptypes["_PFNC_DOUBLE_COMPLEX_TYPE"]
            assert isinstance(dtype, netCDF4._netCDF4.CompoundType)
            assert array_equal(ds["data"][:], complex_array)

        with netCDF4.Dataset(tmp_netcdf, "r", auto_complex=False) as ds:
            dtype = ds.cmptypes["_PFNC_DOUBLE_COMPLEX_TYPE"]
            assert isinstance(dtype, netCDF4._netCDF4.CompoundType)
...
@pytest.mark.local
def test_filters(tmp_netcdf, read_backend):
...
@pytest.mark.local
def test_attributes_list(tmp_netcdf, attr, write_backend, read_backend):
    # regression test for https://github.com/h5netcdf/h5netcdf/issues/291
    with h5netcdf.File(tmp_netcdf, mode="w", backend=write_backend.name) as hf:
        hf.attrs["foo"] = attr

    with h5netcdf.File(tmp_netcdf, mode="r", backend=read_backend.name) as hf:
...
@pytest.mark.local
def test_group_dimensions(tmp_netcdf, write_backend):
    # regression test for https://github.com/h5netcdf/h5netcdf/issues/293
    with h5netcdf.File(tmp_netcdf, mode="w", backend=write_backend.name) as f:
...
@requires_h5py
def test_group_dimensions_classic(tmp_local_netcdf, local_write_backend):
    # regression test for https://github.com/h5netcdf/h5netcdf/issues/293
    with h5netcdf.File(
        tmp_local_netcdf,
        mode="w",
        format="NETCDF4_CLASSIC",
        backend=local_write_backend.name,
    ) as f:
        group = f.create_group("data")
        dims = {"y": 3, "x": 3, "z": None, "z1": None}
